<template>
  <div class="sbti-page">
    <section v-if="currentScreen === 'test'" class="screen active">
      <div class="test-wrap card">
        <div class="topbar">
          <div class="progress"><span :style="{ width: `${progressPercent}%` }"></span></div>
          <div class="progress-text">{{ doneCount }} / {{ totalCount }}</div>
        </div>

        <div class="question-list">
          <article v-for="(question, index) in visibleQuestions" :key="question.id" class="question">
            ...
            <div class="options">
              <label v-for="(option, optionIndex) in question.options" :key="`${question.id}-${option.value}`" class="option">
                <input
        ...
        <div class="actions-bottom">
          <div class="hint">{{ isComplete ? '都做完了。现在可以把你的电子魂魄交给结果页审判。' : '全选完才会放行。世界已经够乱了，起码把题做完整。' }}</div>
          <div class="button-row">
            <button class="btn-secondary" @click="restartTest">重新开始</button>
            <button class="btn-primary submit-button" :disabled="!isComplete" @click="renderResult">提交并查看结果</button>
          </div>
        </div>
      </div>
    </section>

    <section v-else class="screen active">
      <div class="result-wrap card">
        <div class="result-layout">
          <div class="result-top">
            <div :class="['poster-box', { 'no-image': !resultImageSrc }]">
              <img v-if="resultImageSrc" :src="resultImageSrc" class="poster-image" :alt="`${result.finalType.code}（${result.finalType.cn}）`" />
          ...
          <div class="dim-box">
            ...
            <div class="dim-list">
              <div v-for="dim in dimensionOrder" :key="dim" class="dim-item">
                ...
                <p>{{ DIM_EXPLANATIONS[dim][result.levels[dim]] }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="note-box">
            <h3>友情提示</h3>
            <p>{{ result.special ? '本测试仅供娱乐。隐藏人格和傻乐兜底都属于作者故意埋的损招，请勿把它当成医学、心理学、相学、命理学或灵异学依据。' : '本测试仅供娱乐，别拿它当诊断、面试、相亲、分手、招魂、算命或人生判决书。你可以笑，但别太当真。' }}</p>
          </div>
...
const TYPE_LIBRARY = {
  ...
  SEXY: { code: 'SEXY', cn: '尤物', intro: '您就是天生的尤物！', desc: '当您走进一个房间，照明系统会自动将您识别为天生的尤物，并自觉调暗亮度，以避免能源浪费。当您微笑时，您就变成了微笑着的尤物，周围的空气湿度也会显著下降，因为水蒸气都凝结成了人眼中的爱心。无论是谁，都容易对您的存在产生一种超标的注意力。传说，如果有足够多的SEXY人格聚集在一起开派对，其释放出的综合魅力能量能足以暂时扭曲时空结构，让参加者产生“时间变慢了”的幸福错觉。他们不需要卖力表达，很多时候，单是存在本身就已经很像一篇华丽到过分的赋。' },
...
const TYPE_IMAGES = {
  IMSB: '/images/sbti/IMSB.png',
  BOSS: '/images/sbti/BOSS.png',
  MUM: '/images/sbti/MUM.png',
  FAKE: '/images/sbti/FAKE.png',
  'Dior-s': '/images/sbti/Dior-s.jpg',
  DEAD: '/images/sbti/DEAD.png',
  ZZZZ: '/images/sbti/ZZZZ.png',
  GOGO: '/images/sbti/GOGO.png',
  FUCK: '/images/sbti/FUCK.png',
  CTRL: '/images/sbti/CTRL.png',
  HHHH: '/images/sbti/HHHH.png',
  SEXY: '/images/sbti/SEXY.png',
  OJBK: '/images/sbti/OJBK.png',
  'JOKE-R': '/images/sbti/JOKE-R.jpg',
  POOR: '/images/sbti/POOR.png',
  'OH-NO': '/images/sbti/OH-NO.png',
  MONK: '/images/sbti/MONK.png',
  SHIT: '/images/sbti/SHIT.png',
  'THAN-K': '/images/sbti/THAN-K.png',
  MALO: '/images/sbti/MALO.png',
  'ATM-er': '/images/sbti/ATM-er.png',
  'THIN-K': '/images/sbti/THIN-K.png',
  SOLO: '/images/sbti/SOLO.png',
  'LOVE-R': '/images/sbti/LOVE-R.png',
  'WOC!': '/images/sbti/WOC.png',
  DRUNK: '/images/sbti/DRUNK.png',
  IMFW: '/images/sbti/IMFW.png'
}
...
export default {
  ...
  tdk: {
    title: 'SBTI 人格测试',
    description: '在线体验 SBTI 人格测试，完成题目后查看你的娱乐向人格结果与维度分析。',
    keywords: 'SBTI, 人格测试, 心理测试, 娱乐测试'
  },
  data () {
    return {
      currentScreen: 'test',
  ...
  computed: {
    visibleQuestions () {
      ...
      if (gateIndex !== -1 && this.answers.drink_gate_q1 === 3) {
        visible.splice(gateIndex + 1, 0, specialQuestions[1])
      }
      return visible
    },
    doneCount () {
      return this.visibleQuestions.filter(q => this.answers[q.id] !== undefined).length
    },
    totalCount () {
      return this.visibleQuestions.length
    },
    progressPercent () {
      return this.totalCount ? (this.doneCount / this.totalCount) * 100 : 0
    },
    isComplete () {
      return this.totalCount > 0 && this.doneCount === this.totalCount
    },
    resultImageSrc () {
      if (!this.result?.finalType?.code) return ''
      return TYPE_IMAGES[this.result.finalType.code] || ''
    }
  },
  mounted () {
    this.startTest()
  ...
  methods: {
    ...
    getQuestionMetaLabel (question) {
      if (question.special) return '补充题'
      return this.previewMode ? dimensionMeta[question.dim].name : '维度已隐藏'
    },
    ...
    restartTest () {
      this.startTest(this.previewMode)
    },
    ...
    getDrunkTriggered () {
      return this.answers[DRUNK_TRIGGER_QUESTION_ID] === 2
    },
    computeResult () {
      const rawScores = {}
      const levels = {}
      ...
      const ranked = NORMAL_TYPES.map(type => {
        const vector = this.parsePattern(type.pattern).map(level => this.levelNum(level))
        let distance = 0
        let exact = 0
      ...
      const drunkTriggered = this.getDrunkTriggered()
